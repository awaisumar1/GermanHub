...
export function InteractivePractice({ quiz }: InteractivePracticeProps) {
  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-xl p-6 sm:p-8 border border-[var(--color-border-subtle)] my-6 shadow-sm">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="font-semibold text-[var(--color-text)]">Practice: Gap-Fill</h3>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
        <AnimatePresence mode="wait">
          {status === "success" && quiz.explanation ? (
            <motion.div
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
              className="flex items-start gap-2 text-sm text-green-400/90 max-w-md"
            >
            ...
            >
              ...
              <span>Try again! Pay attention to the case.</span>
            </motion.div>
          ) : (
            <div /> // Spacer
          )}
